...
pslist = np.linspace(0,0.15, num = gridnr)
pmlist = np.linspace(0,0.15, num = gridnr)
...
for sc, ps in enumerate(pslist):
    ...
    S3_fHHI = S3_depII @ S3_IdepI @ sef.S3_HII @ sef.S3_IHI
    S3_fHIH = S3_depII @ S3_IIdep @ sef.S3_HII @ sef.S3_IIH
    S3_fIHH = S3_IdepI @ S3_IIdep @ sef.S3_IHI @ sef.S3_IIH
    ...
    for mc, pm in enumerate(pmlist):
        ...
        #%%
        S2_fCX = S2_depI @ S2_Idep @ sef.S2_CX
        
        
        S3_fCXI = S3_depII @ S3_IdepI @ sef.S3_CXI
        S3_fCIX = S3_depII @ S3_IIdep @ sef.S3_CIX
        S3_fICX = S3_IdepI @ S3_IIdep @ sef.S3_ICX
        ...
        fiddiff_tot[sc,mc] = np.sqrt((c2nft_error[0,0]-Fidnft_meas)**2 + (c2ft_error[0,0]-Fidft_meas)**2)
...
#%%
pt.plot_contour_fid(pslist,pmlist,fidnft,'NFT Fidelities')
pt.plot_contour_fid(pslist,pmlist,fidft,'FT Fidelities')
pt.plot_contour_fid(pslist,pmlist,tdnft,'NFT Trace Distance')
pt.plot_contour_fid(pslist,pmlist,tdft,'FT Trace Distance')
